...
class TypeBusinessController extends Controller
{
    ...
    public function list(Request $request){       
        ...
        $sqlx="SELECT id, name FROM type_business WHERE name  like '%$search%' order by id limit 8 offset $curre_pagex";
        $datax=DB::select($sqlx);
        
        $sqltotal="SELECT count(id) as total from type_business where name  like '%$search%' ";
