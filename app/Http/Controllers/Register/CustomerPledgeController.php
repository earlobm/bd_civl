...
use poi\EntityClass\CustomerPledge;
...
class CustomerPledgeController extends Controller
{
    ...
    public function getDatabyNroDoc(Request $request){
        if(isset($request->id)){
            $id=$request->id;
            $sqlx="SELECT p.id, c.id as id_customer_pledge, c.code, p.id_type_document, p.number_doc, p.paternal_last_name, p.maternal_last_name, p.names,
            p.phone, p.address, p.birthdate, p.sex, p.marital_status, p.email, p.reference, p.id_district,di.id_province, pr.id_department
            FROM person p 
            inner join customer_pledge c on c.id_person=p.id
    ...
    public function list_customer(Request $request){
           ...
           $sqlx="SELECT p.id, p.code, p.id_type_document, p.number_doc,p.names,p.paternal_last_name,
            ...
            c.id as id_customer_pledge,
            (select count(p.id) from person p inner join customer_pledge c on c.id_person=p.id where
            c.state=1 and  concat(p.number_doc,' ',p.names,' ',p.paternal_last_name,' ',p.maternal_last_name) 
             like '%$buscar%') AS total from person p inner join customer_pledge c on c.id_person=p.id where c.state=1 and  concat(p.number_doc,' ',p.names,' ',p.paternal_last_name,' ',p.maternal_last_name)  like '%$buscar%' order by c.id limit 8 offset $curre_pagex";
    ...
    public function delete(Request $request){
        ...
        $clasex = CustomerPledge::findOrFail($request->id);
        $clasex->state = 0;
    ...
    public function save(Request $request){
        ...
        if($request->idcustomerpledge==-1){
            $clase_customer = new CustomerPledge();
            $clase_customer->code = $request->code;
        }else{
            $clase_customer = CustomerPledge::findOrFail($request->idcustomerpledge);
        }
    ...
    public function generate_code(Request $request){    
        ...
        FROM person p INNER JOIN customer_pledge cp ON cp.id_person=p.id";
        $miArrayx=DB::select($sqlx);
        $miArray = json_decode(json_encode($miArrayx), true);
        // if(count($miArray)>0){
